...
h3_load_env() {
  ...
  if [[ -f "$env_file" ]]; then
    ...
    local -a preserved=(HEAD_HOST WORKER_HOST HEAD_IP WORKER_IP H3_QUANTIZATION MINIMAX_H3_MODEL_DIR H3_EXECUTION_MODE H3_INT8_W8A8 H3_INT8_DEBUG H3_INT8_EAGER H3_VAE_DECODER_TILE_SIZE H3_VAE_DECODER_TILE_OVERLAP H3_VAE_STACK_TILING H3_TORCH_PROFILER_DIR MINIMAX_H3_TORCH_SDPA_BACKEND)
    local var saved
