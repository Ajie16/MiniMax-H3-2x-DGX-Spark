...
def _non_int8_quant_config(
    quant_config: QuantizationConfig | None,
) -> QuantizationConfig | None:
    """Return None for non-quantized layers when INT8 is enabled."""
    if _USE_INT8 and isinstance(quant_config, ComfyKitchenINT8Config):
        return None
    return quant_config
...
class MiniMaxH3DiTModel(nn.Module):
    ...
    def __init__(
        ...
        self.time_embedder = MiniMaxH3TimeEmbedder(arch, non_int8_config)
        self.rope = MiniMaxH3Rope(arch.rope_inv_freq_len)
        self.token_refiner = MiniMaxH3TokenRefiner(arch, non_int8_config)
        ...
        self.final_layer = MiniMaxH3FinalLayer(arch, non_int8_config)
        if quant_config is not None and not isinstance(quant_config, ComfyKitchenINT8Config):
    ...
    def load_weights(
        ...
        for name, loaded_weight in weights:
            if name.endswith(".comfy_quant"):
                # comfy-kitchen quantization metadata; not a model parameter.
                continue
